...
function TeamButton() {
    ...
    return (
        <>
            <div onClick={() => setBool(!bool)} className='relative w-52 bg-slate-500 rounded-md group z-10 mx-auto'>
                <div className='cursor-pointer rounded-md mt-2 text-center text-white border-b-2 border-black'>Select Team</div>
            </div>
            <div className={`relative w-full h-2/6 bg-blue-950 flex flex-col  ${!debounceIsOpen ? 'hidden' : 'block'} ${isOpen ? 'scale-y-100' : 'scale-y-0'} transition-all origin-top duration-300 rounded z-0 overflow-scroll text-white text-center`}>
                {
                    teams.map((team) => <TeamSearchResult key={team.teamname} teamName={team.teamname} teamAbrv={team.abrv} />)
                }
            </div>
        </>
    )
